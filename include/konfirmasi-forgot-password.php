...
if (isset($_POST['forgot'])) {
  ...
  if ($result->num_rows == 0) {
    header('Location: index.php?include=forgotpassword&error=email_not_registered');
    exit();
  }
  ...
  $mail->Password   = 'wfualbnuvvlewveu'; // Ganti dengan password email Anda
  $mail->SMTPSecure = 'tls';
  $mail->Port       = 587;

  $mail->setFrom('ww.dv.1796@gmail.com', 'Irrigation Website'); // Ganti dengan email dan nama Anda
  ...
  Klik link berikut untuk mereset password Anda: <a href='http://localhost:80/web_irigasi/index.php?include=resetpassword&email=$email&token=$token'>Reset Password</a>";

  try {
    $mail->send();
    header('Location: index.php?include=forgotpassword&success=email_sent');
    exit();
  } catch (Exception $e) {
    header('Location: index.php?include=forgotpassword&error=email_not_sent');
    exit();
  }
}
